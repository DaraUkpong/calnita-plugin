...
export function generateMockRecommendations(userPreferences: any): Product[] {
  ...
  const mockProducts: Product[] = [
    {
      ...
      category: "Skincare",
      price: 49.99,
      image: "https://res.cloudinary.com/df8kki6aw/image/upload/v1716378923/calnita/products/664dcfcf00a84d9a09e0810a/coverPhoto.jpg",
  ...
  const preferredCategory = userPreferences.productPreferences.preferredCategory;
  const ingredientsToAvoid = userPreferences.productPreferences.ingredientsToAvoid;
 
  const dietaryRestrictions = userPreferences.productPreferences.dietaryRestrictions;
  ...
  const generateProduct = (category: string, subcategory: string): Product => {
    ...
    const suitableSkinTypes = SkinCare.SkinType.filter(() => Math.random() > 0.5);
    ...
    const ingredients = ProductPreferences.preferredNaturalIngredients.filter(() => Math.random() > 0.5);
    ...
    return {
      ...
      price,
      image,
      suitableSkinTypes: suitableSkinTypes as ("dry" | "oily" | "combination" | "normal")[],
      targetedConcerns,
      ingredients,
    };
  };
  ...
  const generateProductsForCategory = (category: string, subcategories: string[]) => {
    console.log(subcategories)
    subcategories.forEach((subcategory) => {
      if (Math.random() > 0.5) {
        const product = generateProduct(category, subcategory);
        if (!product.ingredients.some((ingredient) => ingredientsToAvoid.includes(ingredient))) {
  ...
  switch (preferredCategory) {
    case "Skincare":
      ...
      mockProducts.push(...mockProducts.filter(p => p.category === "Skincare"));
      break;
    case "Haircare":
      generateProductsForCategory("Haircare", HairCare.regularProducts);
      mockProducts.push(...mockProducts.filter(p => p.category === "Haircare"));
      break;
    case "Cosmetics":
    ...
    case "Fragrance":
      ...
      mockProducts.push(...mockProducts.filter(p => p.category === "Fragrance"));
      break;
    default:
  ...
  return mockProducts.filter((product) => {
    ...
    return dietaryRestrictions.some((restriction: string) => {
        switch (restriction) {
          ...
          case "Organic":
            return !product.ingredients.some(i => i.toLowerCase().includes("organic"));
          case "Non-GMO":
            return product.ingredients.includes("GMO");
          case "Natural Ingredients":
            return !product.ingredients.some(i => ProductPreferences.preferredNaturalIngredients.includes(i));
          default:
            return false;
        }
      });
...
export function getUniqueFilters(filteredProducts: Product[]) {
  ...
  const ingredients = Array.from(new Set(filteredProducts.flatMap(p => p.ingredients)));

  return { categories, brands, ingredients };
}

// Function to filter products based on selected filters
export function filterProductsByCriteria(products: Product[], selectedFilters: { category?: string[], brand?: string[], ingredient?: string[] }) {
  return products.filter(product => {
    const categoryMatch = !selectedFilters.category || selectedFilters.category.includes(product.category);
    const brandMatch = !selectedFilters.brand || selectedFilters.brand.includes(product.brand);
    const ingredientMatch = !selectedFilters.ingredient || product.ingredients.some(ingredient => selectedFilters.ingredient!.includes(ingredient));
